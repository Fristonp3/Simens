...
static void app_protocol_send_sample(const app_context_t *ctx)
{
    ...
    srv_protocol_send_frame(APP_CMD_READ_SAMPLE, payload, sizeof(payload));
}
...
static void app_protocol_send_status(const app_context_t *ctx)
{
    ...
    if(ctx->status.sleep_armed) {
        flags |= 0x04U;
    }
    if(ctx->status.uart_online) {
        flags |= 0x08U;
    }
    if(ctx->status.boot_requested) {
        flags |= 0x10U;
    }
    ...
    srv_protocol_send_frame(APP_CMD_QUERY_STATUS, payload, sizeof(payload));
}
...
static void app_protocol_handle_read_history(app_context_t *ctx, const srv_protocol_frame_t *frame)
{
    ...
    if(frame->length < 3U) {
        srv_protocol_send_status(APP_CMD_READ_HISTORY, APP_RESULT_ARG);
        return;
    }

    start = app_protocol_read_u16_be(&frame->payload[0]);
    requested = frame->payload[2];
    if(requested == 0U) {
        requested = 1U;
    }
    if(requested > PROJECT_HISTORY_BURST_MAX) {
        requested = PROJECT_HISTORY_BURST_MAX;
    }

    count = srv_storage_read_range(start, requested, records);
    ...
    srv_protocol_send_frame(APP_CMD_READ_HISTORY, response, (uint8_t)offset);
...
void app_protocol_poll(app_context_t *ctx, uint32_t now_ms)
{
    ...
    while(srv_protocol_poll(&frame)) {
        ...
        case APP_CMD_SET_DAC:
            if(frame.length != 2U) {
                srv_protocol_send_status(APP_CMD_SET_DAC, APP_RESULT_ARG);
            } else {
                ctx->config.dac_raw = app_protocol_read_u16_be(frame.payload);
                bsp_dac_set_raw(ctx->config.dac_raw);
                srv_settings_save(&ctx->config);
                ctx->display_dirty = true;
                srv_protocol_send_status(APP_CMD_SET_DAC, APP_RESULT_OK);
            }
            break;

        case APP_CMD_SET_TIME:
            if(frame.length != 6U) {
                srv_protocol_send_status(APP_CMD_SET_TIME, APP_RESULT_ARG);
            } else {
                rtc_datetime_t time = {
                ...
                ctx->display_dirty = true;
                srv_protocol_send_status(APP_CMD_SET_TIME, APP_RESULT_OK);
            }
            break;
        ...
        case APP_CMD_GET_TIME:
        {
            ...
            srv_protocol_send_frame(APP_CMD_GET_TIME, payload, sizeof(payload));
            break;
        }
        ...
        case APP_CMD_START_RECORD:
            ...
            srv_protocol_send_status(APP_CMD_START_RECORD, APP_RESULT_OK);
            break;

        case APP_CMD_STOP_RECORD:
            (void)app_state_set(ctx, APP_STATE_MONITOR, now_ms);
            srv_protocol_send_status(APP_CMD_STOP_RECORD, APP_RESULT_OK);
            break;
        ...
        case APP_CMD_ERASE_HISTORY:
            ...
            srv_protocol_send_status(APP_CMD_ERASE_HISTORY, APP_RESULT_OK);
            break;
        ...
        case APP_CMD_ENTER_BOOT:
            ...
            srv_protocol_send_status(APP_CMD_ENTER_BOOT, APP_RESULT_OK);
            break;

        case APP_CMD_SWITCH_MODE:
            if((frame.length != 1U) || (!app_state_set_by_mode_id(ctx, frame.payload[0], now_ms))) {
                srv_protocol_send_status(APP_CMD_SWITCH_MODE, APP_RESULT_ARG);
            } else {
                srv_protocol_send_status(APP_CMD_SWITCH_MODE, APP_RESULT_OK);
            }
            break;

        default:
            srv_protocol_send_status(frame.command, APP_RESULT_ERROR);
            break;
        }
    }
}
